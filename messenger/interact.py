...
class User:
    def __init__(self, name, password, level):
        ...
        self.level = control.Control[level]
...
class Interact:
    ...
    def __init__(self, username, password, messages):
        ...
        self._user = users[self._id_from_user(username)] # Store user to access privilege level

    ##################################################
    # INTERACT :: SHOW
    # Show a single message if user has >= privilege
    ################################################## 
    def show(self):
        id_ = self._prompt_for_id("display")
        message = self._p_messages.get_message(id_) # use id get message for checking

        if control.canRead(self._user.level, message.get_privilege()):
